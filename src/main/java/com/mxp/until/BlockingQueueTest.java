 ...
 * User : mxp
 * Mail : menxipeng@gmail.com
 * Date : 2018/9/27
 * Time : 14:57
 **/
public class BlockingQueueTest{

    private static ArrayBlockingQueue<Integer> queue = new ArrayBlockingQueue<Integer>(10,true);
        ...
        public void run() {
            try {
                while(true){
                    ...
                    //offer方法为非堵塞的
                    //queue.offer(rnd.nextInt(100), 2, TimeUnit.SECONDS); //等待1秒后还不能加入队列则返回失败，放弃加入
                    //queue.offer(rnd.nextInt(100));

                    producerLatch.countDown(); //state值减1
                    System.out.println("state====>"+producerLatch);
